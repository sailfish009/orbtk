...
pub fn update_state(state: &str, flag: bool, selector: &mut crate::theme::Selector) {
    if flag
        && (selector.state.is_none()
            || (selector.state.is_some() && !selector.state.as_ref().unwrap().eq(state)))
    {
